...
def resource_path(relative_path):
    """Obtenir le chemin absolu pour une ressource."""
    try:
...
# Liste des musiques et index pour la musique actuelle
music_files = [
    resource_path("musique_fond.mp3"),
    resource_path("Electroman Adventures.mp3"),
    resource_path("Miss Alissa.mp3"),
    resource_path("Chopin.mp3"),
    resource_path("Troll.mp3"),
    resource_path("Interstellar.mp3"),
]
...
def play_music():
    """Lecture de la musique en fonction de l'index actuel."""
    if music_enabled:
        pygame.mixer.music.load(music_files[current_music_index])
        pygame.mixer.music.play(-1)  # Lecture en boucle

def next_music():
    """Change pour la musique suivante."""
    global current_music_index
    current_music_index = (current_music_index + 1) % len(music_files)
    play_music()

def previous_music():
    """Change pour la musique précédente."""
    global current_music_index
    current_music_index = (current_music_index - 1) % len(music_files)
    play_music()
...
def start_screen():
    ...
    # Joue la musique selon l'indice actuel
    play_music()
    ...
    texts = {
        "FR": {
            ...
            "design": "Appuyez sur D pour personnaliser les couleurs",
        ...
        "EN": {
            ...
            "design": "Press D to Customize Colors",
    ...
    while True:
        ...
        # Charger la police
        try:
            font = pygame.font.Font(resource_path("PressStart2P.ttf"), int(32 * SCALE_FACTOR))
        except FileNotFoundError:
        ...
        design_text = font.render(lang_text["design"], True, WHITE)
        ...
        design_text_rect = design_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 20))
        quit_text_rect = quit_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 20))
        
        # Meilleurs scores centrés
        score_text_easy_rect = score_text_easy.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 120))
        score_text_medium_rect = score_text_medium.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 160))
        score_text_hard_rect = score_text_hard.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 200))

        # Dessin des éléments sur l'écran
        screen.blit(title, title_rect)
        screen.blit(play_text, play_text_rect)
        screen.blit(settings_text, settings_text_rect)
        screen.blit(design_text, design_text_rect)
...
def design_menu():
    """
    Menu de personnalisation des couleurs pour le serpent et la pomme.
    """
    global snake_color, apple_color, language
    colors = [WHITE, GREEN, RED, BLACK, (255, 165, 0), (0, 0, 255), (128, 0, 128)]  # Liste de quelques couleurs
    color_index_snake = colors.index(snake_color)
    color_index_apple = colors.index(apple_color)
    ...
    texts = {
        "FR": {
            ...
            "apple_color": "Couleur de la Pomme (←/→):",
            "exit": "Appuyez sur Echap pour retourner"
        },
        "EN": {
    ...
    lang_text = texts[language]  # Charger les textes en fonction de la langue sélectionnée
    ...
    while True:
        ...
        # Instructions pour changer les couleurs en fonction de la langue
        title = font.render(lang_text["title"], True, WHITE)
        snake_text = font.render(lang_text["snake_color"], True, WHITE)
        apple_text = font.render(lang_text["apple_color"], True, WHITE)
        exit_text = font.render(lang_text["exit"], True, WHITE)
        
        # Positionnement du texte
        title_rect = title.get_rect(center=(SCREEN_WIDTH // 2, 100))
        snake_text_rect = snake_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 100))
        apple_text_rect = apple_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 60))
        exit_text_rect = exit_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT - 100))
        
        # Affichage des exemples de serpent et de pomme avec les couleurs actuelles
        for i in range(5):  # Exemple de serpent de 5 cases
            pygame.draw.rect(screen, snake_color, pygame.Rect(SCREEN_WIDTH // 2 - 100 + i * CELL_SIZE, SCREEN_HEIGHT // 2, CELL_SIZE, CELL_SIZE))
        pygame.draw.rect(screen, apple_color, pygame.Rect(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 100, CELL_SIZE, CELL_SIZE))  # Exemple de pomme
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    return  # Sortir du menu pour revenir à l'accueil
                elif event.key == pygame.K_UP:
                    color_index_snake = (color_index_snake + 1) % len(colors)
                    snake_color = colors[color_index_snake]
                elif event.key == pygame.K_DOWN:
                    color_index_snake = (color_index_snake - 1) % len(colors)
                    snake_color = colors[color_index_snake]
                elif event.key == pygame.K_LEFT:
                    color_index_apple = (color_index_apple - 1) % len(colors)
                    apple_color = colors[color_index_apple]
...
def settings_menu():
    ...
    line_spacing = 80  # Espacement entre les lignes pour améliorer la lisibilité
    ...
    while True:
        ...
        texts = {
            "FR": {
                ...
                "change_music": "(↑/↓) pour changer la musique",
            ...
            "EN": {
                ...
                "change_music": "UP/DOWN to change music",
                "volume_control": f"Volume: {int(volume_level * 100)}% (← / →)",
...
def pause_menu():
    ...
    # Textes de pause en fonction de la langue
    texts = {
        "FR": {
            "pause": "Jeu en Pause",
            "resume": "Appuyez sur Espace pour reprendre",
            "quit": "Appuyez sur Q pour quitter la partie"
        },
        "EN": {
            "pause": "Game Paused",
    ...
    while paused:
        ...
        # Texte de pause en fonction de la langue
        pause_text = font.render(lang_text["pause"], True, WHITE)
        resume_text = font.render(lang_text["resume"], True, WHITE)
        quit_text = font.render(lang_text["quit"], True, WHITE)
...
def game_over():
    """
    Fonction de fin de jeu pour afficher Game Over et reprendre la musique sélectionnée.
    """
    global best_scores, score, difficulty, music_started

    # Si le son de game over est configuré, le jouer ici
    if game_over_sound:
    ...
    # Affichage de Game Over avec le score et le meilleur score
    while True:
        screen.fill(BLACK)
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:  # Retourner à l'écran d'accueil
                    # Rejoue la musique actuelle lors du retour à l'écran d'accueil
                    if music_enabled:
                        play_music()
                    main()
                    return
